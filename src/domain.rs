...
pub const ORGANIZATION: &str = "led-dynamo";
pub const REPOSITORY: &str = "leddy-mcp-server.rs";
pub const DOMAIN_SUMMARY: &str = "Read-only MCP diagnostics for LED displays, framebuffers, commands, telemetry, and fleet topology";
pub const UNIT_LABEL: &str = "display units";

const REPOSITORIES: [(&str, &str); 5] = [
    (
        "leddy-api-server.rs",
        "message, display, and telemetry command plane",
    ),
    (
        "leddy-interfaces",
        "display, command, message, and telemetry contracts",
    ),
    (
        "leddy-lib",
        "framebuffer, font, layout, and scrolling renderer",
    ),
...
impl Workload {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::FramebufferRender => "framebuffer_render",
            Self::MessageFanout => "message_fanout",
            Self::DeviceTelemetry => "device_telemetry",
        }
    }
}
...
pub struct PlanInput {
    ...
    /// Bounded number of display units to evaluate.
...
pub const fn workload_names() -> [&'static str; 3] {
    ["framebuffer_render", "message_fanout", "device_telemetry"]
}
