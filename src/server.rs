...
pub const SERVER_NAME: &str = "leddy-mcp-server";
pub const SERVER_NAMESPACE: &str = "led-dynamo";

#[derive(Clone)]
pub struct LeddyMCPServer {
...
impl LeddyMCPServer {
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "leddy_fleet_map", mcp.tool.class = "inventory"))]
    fn leddy_fleet_map(&self) -> String {
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "leddy_plan", mcp.tool.class = "details"))]
    fn leddy_plan(&self, Parameters(input): Parameters<PlanInput>) -> Result<String, String> {
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "leddy_runtime_readiness", mcp.tool.class = "health"))]
    fn leddy_runtime_readiness(&self) -> String {
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "leddy_shared_platform", mcp.tool.class = "inventory"))]
    fn leddy_shared_platform(&self) -> String {
        let timer = self.metrics.start(ToolClass::Inventory);
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "leddy_lifecycle_state", mcp.tool.class = "health"))]
    fn leddy_lifecycle_state(&self) -> Result<String, String> {
        let timer = self.metrics.start(ToolClass::Health);
        let result = self
            .lifecycle
            .snapshot_and_audit()
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "leddy_safety_boundary", mcp.tool.class = "inventory"))]
    fn leddy_safety_boundary(&self) -> String {
        let timer = self.metrics.start(ToolClass::Inventory);
...
impl ServerHandler for LeddyMCPServer {
    ...
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new(SERVER_NAME, env!("CARGO_PKG_VERSION")).with_title("Leddy MCP Server"))
            .with_instructions("Read-only MCP diagnostics for LED displays, framebuffers, commands, telemetry, and fleet topology. The server is read-only and never logs MCP arguments or results.")
    }
}
...
mod tests {
    ...
    fn exact_prefixed_tool_catalog_is_exposed() {
        let server = LeddyMCPServer::new(LifecycleController::new(8).expect("valid lifecycle"));
        let names = server
            .tool_router
            .list_all()
            .into_iter()
