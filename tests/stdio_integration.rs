...
async fn real_process_negotiates_and_lists_namespaced_tools_without_stdout_noise() {
    let mut child = tokio::process::Command::new(env!("CARGO_BIN_EXE_leddy-mcp-server"))
...
"
    ...
    assert!(serialized.contains("leddy_fleet_map"));
    assert!(serialized.contains("leddy_plan"));
}
